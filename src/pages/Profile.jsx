...
import {
    Trophy, CheckCircle2, XCircle, Activity, Search,
...
// const API_DATA = {
//   user: {
//     _id: "69b436153350e5481bc9288e",
//     firstName: "Abhinav",
//     emailId: "abhinavchoubey22@gmail.com",
//     role: "admin",
//     totalProblemSolved: 245 // Increased for better visual representation
//   },
//   submissions: [
//     { id: 1, title: "Two Sum", status: "Accepted", runtime: 45, memory: 12.4, createdAt: "2023-10-01T10:00:00Z" },
//     { id: 2, title: "Add Two Numbers", status: "Wrong Answer", runtime: 0, memory: 0, createdAt: "2023-10-02T11:30:00Z" },
//     { id: 3, title: "Longest Substring", status: "Accepted", runtime: 72, memory: 14.1, createdAt: "2023-10-04T09:15:00Z" },
//     { id: 4, title: "Median of Two Arrays", status: "Accepted", runtime: 110, memory: 18.5, createdAt: "2023-10-05T14:20:00Z" },
//     { id: 5, title: "Reverse Integer", status: "Wrong Answer", runtime: 0, memory: 0, createdAt: "2023-10-07T16:45:00Z" },
//     { id: 6, title: "Palindrome Number", status: "Accepted", runtime: 58, memory: 13.2, createdAt: "2023-10-08T10:10:00Z" },
//     { id: 7, title: "Regular Expression", status: "Accepted", runtime: 85, memory: 15.8, createdAt: "2023-10-10T12:00:00Z" },
//     { id: 8, title: "Container With Water", status: "Accepted", runtime: 63, memory: 12.9, createdAt: "2023-10-12T18:30:00Z" },
//     { id: 9, title: "Integer to Roman", status: "Accepted", runtime: 42, memory: 11.5, createdAt: "2023-10-15T08:20:00Z" },
//     { id: 10, title: "3Sum", status: "Wrong Answer", runtime: 0, memory: 0, createdAt: "2023-10-18T21:00:00Z" },
//   ],
...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axiosMain.get("user/getProfile");
    ...
    const pieData = [
        { name: 'Accepted', value: submissions.filter(s => s.status === 'Accepted').length, color: '#10b981' },
        { name: 'Wrong Answer', value: submissions.filter(s => s.status === 'Wrong Answer').length, color: '#f43f5e' },
    ];

    const acceptanceRate = submissions.length
        ? ((pieData[0].value / submissions.length) * 100).toFixed(1)
        : 0;
        ...
        <div className="min-h-screen bg-[#0f172a] text-slate-200 p-4 md:p-8 font-sans selection:bg-blue-500/30">
            ...
            <main className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <StatsCard index={1} title="Accepted" value={pieData[0].value} icon={CheckCircle2} color="bg-emerald-500/10" />
                    <StatsCard index={2} title="Wrong Answers" value={pieData[1].value} icon={XCircle} color="bg-rose-500/10" />
                    <StatsCard index={3} title="Acceptance Rate" value={`${acceptanceRate}%`} icon={Zap} color="bg-purple-500/10" />
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <Card className="p-6 flex flex-col items-center justify-center">
                        <h3 className="text-lg font-semibold mb-4 self-start">Success Distribution</h3>
                ...
                <Card className="p-0">
                    ...
                    <div className="p-6 border-t border-slate-800 flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                            <button className="p-2 rounded-lg border border-slate-700 hover:bg-slate-800 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                                <ChevronLeft size={20} />
                            </button>
                            {[1, 2, 3, 4].map(num => (
                                <button
                                    key={num}
                                    className={`w-10 h-10 rounded-lg text-sm font-medium transition-all ${pagination.page === num ? 'bg-blue-600 text-white' : 'border border-slate-700 hover:bg-slate-800'}`}
                                >
                                    {num}
                                </button>
                            ))}
                            <button className="p-2 rounded-lg border border-slate-700 hover:bg-slate-800 transition-all">
